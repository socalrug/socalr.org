...
clean_events <- function(events_data) {
  months <- lubridate::month(events$time)
  month_abbs <- month.abb[months]
  days <- lubridate::day(events$time)
...
as_link <- function(link, icon, ...) {
  tags$a(style = "text-decoration:none", href = link, fa(icon, height = "2rem", fill = "steelblue"), ...)
}
...
fa_row <- function(group_name, website, meetup_page, twitter, github, slack) {
  as_ul(as_group_name(group_name), as_website(website), as_meetup(meetup_page), as_twitter(twitter), as_github(github), as_slack(slack))
}

fa_rows <- laocrugs %>%
  select(group_name, website, meetup_page, twitter, github, slack) %>% 
  pmap(fa_row)
```
...
```{r, layout="l-screen-inset", cache=TRUE}
library(leaflet)
library(ggmap)
ggmap::register_google("AIzaSyBFkf5o3bW7blAGb_E7-ZNhxk3QBFtqQko")
